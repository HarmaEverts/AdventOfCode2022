...
def find_large_directories(directory, limit):
    size = directory.get_directory_size()
    if size <= limit:
        result_list.append(directory)
    if directory.has_subdirectory():
        for subdir in directory.get_subdirectories():
            find_large_directories(subdir, limit)
...
with open('day7_input.txt', 'r') as f:
    ...
    result_list = []
    find_large_directories(filesystem, 100000)
    # print_dir_tree(filesystem, 0)
    result = 0
    for res in result_list:
        result += res.get_directory_size()
    print(result)
